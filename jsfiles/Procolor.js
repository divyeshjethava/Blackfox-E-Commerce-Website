...
var photo1;
  ...
  document.addEventListener('DOMContentLoaded', () => { 
    ...
    const data = urlParams.get('veid');
    const mpid = urlParams.get('pid');
   var cd = data;
    const popular_mostref = ref(db,'Products/'+mpid+'/'+'varition/' +data);
...
get(popular_mostref).then((snapshot) => {
  if (snapshot.exists()) {
   ...
   photo1 = data.sub_image; 
    ...
    //const qtty = data.quantity;
    console.log('tit'+p_price)
...
const popular_mostref3 = ref(db, 'wishlist/' + phoneNumber1 + '/' + cd );
var l = f_status;
...
const popular_mostref1 = ref(db, 'Products/' + mpid );
get(popular_mostref1).then(snapshot => {
    const data = snapshot.val(); 
   const clr = data.colour
...
colorsArray.forEach((color1) =>{
    ...
    clrdiv1.addEventListener('click',() =>{
      ...
      window.location.href='productsColor.html?pid='+mpid+"&"+"veid="+color1.veriti;
    })
  
    const veidParam = getUrlParameter('veid');
  if (veidParam === color1.veriti) {
    clrdiv1.style.border = '2px solid black'; 
  }
  })
 ...
 const clrdiv1 = document.createElement('div');
    clrdiv1.style.background=clr;
    clrdiv1.classList.add('clrdiv1');
    clrdiv.appendChild(clrdiv1);
    clrdiv1.addEventListener('click',function(){
        window.location.href = "proDeatils.html?id="+mpid;
    })
});
...
acart.addEventListener('click',() => {
 ...
 selectSizeMes.style.display='initial';
    ...
    var phoneNumber1 = localStorage.getItem('phoneNumber');
    const popular_mostref1 = ref(db, 'bag/' + phoneNumber1 + '/' + cd );
    const popular_mostref2 = ref(db, 'Products/' + mpid +'/'+ 'varition');
    var qty =document.getElementById('pqty');
    get(popular_mostref2).then((snapshot) => {
      if (snapshot.exists()) {
        const data1 = snapshot.val(); 
       var photo1 = data.cover_image; 
         var f_status = data.f_status;
         var p_name = data.p_name;
         var p_price = data.price;
         var tprice = p_price * qty.value;
         var clrs = data.colorname;
